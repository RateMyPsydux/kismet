...
class KismetRtl433(object):
    ...
    async def __rtl_433_task(self):
        ...
        try:
            ...
            while True:
                ...
                if pipe_fd == -1:
                    print("Failed to open the named pipe!")
                    exit(1)

                received_json_bytes = os.read(pipe_fd, 4096)  # Assuming integer size is 4 bytes

                if received_json_bytes:
                    received_json_str = received_json_bytes.decode('utf-8')
                    print("Received JSON from sender:", received_json_str)
                    self.handle_json(received_json_str)
    ...
    def datasource_listinterfaces(self, seqno):
        ...
        intf.interface = f"Mac"
        intf.capinterface = f"gnumac"
        intf.flags = ""
        intf.hardware = "TEST"
        interfaces.append(intf)

        #interfaces.append("Gnumac")
