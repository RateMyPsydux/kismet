...
class gnumac_tracked_common : public tracker_component {
public:
    gnumac_tracked_common() :
        tracker_component() {
        register_fields();
        reserve_fields(NULL);
    }

    gnumac_tracked_common(int in_id) :
    ...
    virtual uint32_t get_signature() const override {
        return adler32_checksum("gnumac_tracked_common");
    }
...
protected:
    virtual void register_fields() override {
        ...
        register_field("gnumac.device.model", "Sensor model", &model);
        register_field("gnumac.device.id", "Sensor ID", &rtlid);
        register_field("gnumac.device.rtlchannel", "Sensor sub-channel", &rtlchannel);
        register_field("gnumac.device.battery", "Sensor battery level", &battery);
        register_field("gnumac.device.rssi", "Sensor rssi level", &rssi);
        register_field("gnumac.device.snr", "Sensor snr level", &snr);
        register_field("gnumac.device.noise", "Sensor noise level", &noise);
    }
...
class gnumac_tracked_thermometer : public tracker_component {
public:
    gnumac_tracked_thermometer() :
        tracker_component() {
        register_fields();
        reserve_fields(NULL);
    }

    gnumac_tracked_thermometer(int in_id) :
       tracker_component(in_id) {
            register_fields();
            reserve_fields(NULL);
        }

    gnumac_tracked_thermometer(int in_id, std::shared_ptr<tracker_element_map> e) :
        tracker_component(in_id) {
        register_fields();
        reserve_fields(e);
    }
    ...
    virtual uint32_t get_signature() const override {
        return adler32_checksum("gnumac_tracked_thermometer");
    }
    ...
    typedef kis_tracked_rrd<gnumac_empty_aggregator> rrdt;
    __ProxyTrackable(temperature_rrd, rrdt, temperature_rrd);
    __ProxyTrackable(humidity_rrd, rrdt, humidity_rrd);

protected:
    virtual void register_fields() override {
        register_field("gnumac.device.temperature", "Temperature (C)", &temperature);
        register_field("gnumac.device.temperature_rrd", "Temperature history RRD", &temperature_rrd);
        register_field("gnumac.device.humidity", "Humidity (percent)", &humidity);
        register_field("gnumac.device.humidity_rrd", "Humidity history RRD", &humidity_rrd);
    }
    ...
    std::shared_ptr<kis_tracked_rrd<gnumac_empty_aggregator>> temperature_rrd;

    // Basic humidity in percentage, from multiple sensors
    std::shared_ptr<tracker_element_int32> humidity;
    std::shared_ptr<kis_tracked_rrd<gnumac_empty_aggregator>> humidity_rrd;
};

// Weather station type data
class gnumac_tracked_weatherstation : public tracker_component {
public:
    gnumac_tracked_weatherstation() :
        tracker_component() {
        register_fields();
        reserve_fields(NULL);
    }

    gnumac_tracked_weatherstation(int in_id) :
        tracker_component(in_id) {
            register_fields();
            reserve_fields(NULL);
        }

    gnumac_tracked_weatherstation(int in_id, std::shared_ptr<tracker_element_map> e) :
        tracker_component(in_id) {
        register_fields();
        reserve_fields(e);
    }
    ...
    virtual uint32_t get_signature() const override {
        return adler32_checksum("gnumac_tracked_weatherstation");
    }
...
protected:
    virtual void register_fields() override {
        register_field("gnumac.device.wind_dir", "Wind direction (degrees)", &wind_dir);
        register_field("gnumac.device.wind_dir_rrd", "Wind direction RRD", &wind_dir_rrd);

        register_field("gnumac.device.weatherstation.wind_speed", "Wind speed (KPH)", &wind_speed);
        register_field("gnumac.device.wind_speed_rrd", "Wind speed RRD", &wind_speed_rrd);

        register_field("gnumac.device.wind_gust", "Wind gust (KPH)", &wind_gust);
        register_field("gnumac.device.wind_gust_rrd", "Wind gust RRD", &wind_gust_rrd);

        register_field("gnumac.device.rain", "Measured rain", &rain);
        register_field("gnumac.device.rain_rrd", "Rain RRD", &rain_rrd);

        register_field("gnumac.device.uv_index", "UV index", &uv_index);
        register_field("gnumac.device.uv_index_rrd", "UV Index RRD", &uv_index_rrd);

        register_field("gnumac.device.lux", "Lux", &lux);
        register_field("gnumac.device.lux_rrd", "Lux RRD", &lux_rrd);
    }

    // Wind direction in degrees
    std::shared_ptr<tracker_element_int32> wind_dir;
    std::shared_ptr<kis_tracked_rrd<gnumac_empty_aggregator>> wind_dir_rrd;

    // Wind speed in kph (might have to convert for some sensors)
    std::shared_ptr<tracker_element_int32> wind_speed;
    std::shared_ptr<kis_tracked_rrd<gnumac_empty_aggregator>> wind_speed_rrd;

    std::shared_ptr<tracker_element_int32> wind_gust;
    std::shared_ptr<kis_tracked_rrd<gnumac_empty_aggregator>> wind_gust_rrd;

    // Rain (in whatever the sensor reports it in)
    std::shared_ptr<tracker_element_int32> rain;
    std::shared_ptr<kis_tracked_rrd<gnumac_empty_aggregator>> rain_rrd;

    // UV
    std::shared_ptr<tracker_element_int32> uv_index;
    std::shared_ptr<kis_tracked_rrd<gnumac_empty_aggregator>> uv_index_rrd;

    // Lux
    std::shared_ptr<tracker_element_int32> lux;
    std::shared_ptr<kis_tracked_rrd<gnumac_empty_aggregator>> lux_rrd;
};

class gnumac_tracked_lightningsensor : public tracker_component {
public:
    gnumac_tracked_lightningsensor() :
        tracker_component() {
            register_fields();
            reserve_fields(nullptr);
        }

    gnumac_tracked_lightningsensor(int in_id) :
        tracker_component(in_id) {
            register_fields();
            reserve_fields(nullptr);
        }

    gnumac_tracked_lightningsensor(int in_id, std::shared_ptr<tracker_element_map> e) :
        tracker_component(in_id) {
            register_fields();
            reserve_fields(e);
        }
    ...
    virtual uint32_t get_signature() const override {
        return adler32_checksum("gnumac_tracked_lightningsensor");
    }
...
protected:
    ...
    virtual void register_fields() override {
        register_field("gnumac.device.lightning_strike_count", "Strike count", &strike_count);
        register_field("gnumac.device.lightning_storm_distance", "Storm distance (no unit)", &storm_distance);
        register_field("gnumac.device.lightning_storm_active", "Storm active", &storm_active);
        register_field("gnumac.device.lightning_rfi", "Lightning radio frequency interference", &lightning_rfi);
    }
...
class gnumac_tracked_tpms : public tracker_component {
public:
    gnumac_tracked_tpms() :
        tracker_component() {
        register_fields();
    ...
    gnumac_tracked_tpms(int in_id) :
       tracker_component(in_id) {
            register_fields();
            reserve_fields(NULL);
        }

    gnumac_tracked_tpms(int in_id, std::shared_ptr<tracker_element_map> e) :
        tracker_component(in_id) {
        register_fields();
        reserve_fields(e);
    }
    ...
    virtual uint32_t get_signature() const override {
        return adler32_checksum("gnumac_tracked_tpms");
    }
...
protected:
    virtual void register_fields() override {
        register_field("gnumac.device.tpms.pressure_bar", "Pressure, in bars", &pressure_bar);
        register_field("gnumac.device.tpms.pressure_kpa", "Pressure, in kPa", &pressure_kpa);
        register_field("gnumac.device.tpms.flags", "TPMS flags", &flags);
        register_field("gnumac.device.tpms.state", "TPMS state", &state);
        register_field("gnumac.device.tpms.checksum", "TPMS checksum", &checksum);
        register_field("gnumac.device.tpms.code", "TPMS code", &code);
    }
...
// Switch panels
class gnumac_tracked_switch : public tracker_component {
public:
    gnumac_tracked_switch() :
        tracker_component() {
        register_fields();
        reserve_fields(NULL);
    }

    gnumac_tracked_switch(int in_id) :
       tracker_component(in_id) {
            register_fields();
            reserve_fields(NULL);
        }

    gnumac_tracked_switch(int in_id, std::shared_ptr<tracker_element_map> e) :
        tracker_component(in_id) {
        register_fields();
        reserve_fields(e);
    }
    ...
    virtual uint32_t get_signature() const override {
        return adler32_checksum("gnumac_tracked_switch");
    }
...
protected:
    virtual void register_fields() override {
        //register_field("gnumac.device.switch_vec", "Switch settings", &switch_vec);
        //switch_vec_entry_id = 
        //    register_field("gnumac.device.switch.position", 
        //            tracker_element_factory<tracker_element_int32>(),
        //            "Switch position");
        register_field("gnumac.device.switch.1", "Switch 1", &switch1);
        register_field("gnumac.device.switch.2", "Switch 2", &switch2);
        register_field("gnumac.device.switch.3", "Switch 3", &switch3);
        register_field("gnumac.device.switch.4", "Switch 4", &switch4);
        register_field("gnumac.device.switch.5", "Switch 5", &switch5);
    }
...
// Insteon
class gnumac_tracked_insteon : public tracker_component {
public:
    gnumac_tracked_insteon() :
        tracker_component() {
        register_fields();
        reserve_fields(NULL);
    }

    gnumac_tracked_insteon(int in_id) :
       tracker_component(in_id) {
            register_fields();
            reserve_fields(NULL);
        }

    gnumac_tracked_insteon(int in_id, std::shared_ptr<tracker_element_map> e) :
        tracker_component(in_id) {
        register_fields();
        reserve_fields(e);
    }
    ...
    virtual uint32_t get_signature() const override {
        return adler32_checksum("gnumac_tracked_insteon");
    }
...
protected:
    virtual void register_fields() override {
        register_field("gnumac.device.from_id", "From", &from_id);
        register_field("gnumac.device.to_id", "To", &to_id);
        register_field("gnumac.device.msg_type", "Message Type", &msg_type);
        register_field("gnumac.device.msg_str", "Message String", &msg_str);
        register_field("gnumac.device.hopsmax", "Hops Max", &hopsmax);
        register_field("gnumac.device.hopsleft", "Hops Left", &hopsleft);
    }
...
class Kis_GNUMAC_Phy : public kis_phy_handler {
public:
    virtual ~Kis_GNUMAC_Phy();
	...
	virtual kis_phy_handler *create_phy_handler(int in_phyid) override {
		return new Kis_GNUMAC_Phy(in_phyid);
	}

    Kis_GNUMAC_Phy(int in_phyid);
...
protected:
    ...
    int gnumac_holder_id, gnumac_common_id, gnumac_thermometer_id, 
        gnumac_weatherstation_id, gnumac_tpms_id, gnumac_switch_id,
        gnumac_insteon_id, gnumac_lightning_id;
